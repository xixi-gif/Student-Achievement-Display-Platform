...
export const authApi = {
  ...
  uploadAvatar: (file) => {
    ...
    const token = localStorage.getItem("token");
    console.log(
      "uploadAvatar - 从localStorage获取的Token:",
    ...
    return service
      .post(`/user/upload/avatar`, formData, { headers })
      .then((response) => {
  ...
  muteConversation: (conversationId) =>
    service.put(`/conversations/${conversationId}/mute`),
  pinConversation: (conversationId) =>
    service.put(`/conversations/${conversationId}/pin`),
  unmuteConversation: (conversationId) =>
    service.put(`/conversations/${conversationId}/unmute`),
  unpinConversation: (conversationId) =>
    service.put(`/conversations/${conversationId}/unpin`),
  //消息
  getConversationMessages: (conversationId) =>
    service.get(`/messages/conversation/${conversationId}`),
  markAsRead: (conversationId) =>
    service.put(`/messages/conversation/${conversationId}/read`),
  getUnreadCount: (conversationId) =>
    service.get(`/messages/conversation/${conversationId}/unread-count`),
  searchMessages: (data) => service.get("/messages/search", data),
  getTotalUnreadCount: (data) =>
    service.get("/messages/total-unread-count", data),
...
export const authorApi = {
  ...
  getUserPublicInfo: (userId) =>
    service.get(`/user/public/info/?userId=${userId}`),
};
...
export const achievementApi = {
  ...
  createAchievement: (data) =>
    service.post("/achievement/add", data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
      timeout: 60000, // 1分钟超时
  ...
  updateAchievement: (data) =>
    service.post(`/achievement/update`, data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
      timeout: 60000, // 1分钟超时
  ...
  cancelLikeAchievement: (data) =>
    service.post("/achievement/like/cancel", data),
  checkLikeStatus: (data) => service.post("/achievement/like/status", data),

  // 获取待审核成果列表
  getPendingList: (params) => service.get(`/teacher/review`, { params }),
  // 审核通过
  approve: (id) =>
    service.post(`/teacher/review/approve`, null, { params: { id } }),
  // 审核驳回
  reject: (id, reason) =>
    service.post(`/teacher/review/reject`, { id, reason }),
  //获取推荐列表
...
export const adminApi = {
  ...
  resetUserPassword: (id, password) =>
    service.put(`/admin/password`, { password }, { params: { id } }),
  toggleUserStatus: (id, status) =>
    service.post(`/admin/users/${id}/status`, { status }),
  ...
  deleteCategory: (id) =>
    service.post("/admin/categories/delete", null, {
      params: { id },
  ...
  deleteAchievement: (ids) =>
    service.post("/admin/achievement/batch-delete", ids),
  updateAchievementStatus: (params) => {
    const queryParams = new URLSearchParams(); // 将数组参数转换为查询字符串格式
    params.achievementIds.forEach((id) =>
      queryParams.append("achievementIds", id)
    );
    queryParams.append("status", params.status);

    return service.post(
      `/admin/achievement/batch-update-status?${queryParams.toString()}`
    );
  },
  ...
  //轮播图顺序
  adjustSequence: (id, direction) =>
    service.put(`/carousel/${id}/sequence`, null, { params: { direction } }),
