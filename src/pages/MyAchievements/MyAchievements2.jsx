...
const MyAchievements = () => {
  ...
  const filteredAchievements = achievements.filter((achievement) => {
    const matchesSearch =
      achievement.title.toLowerCase().includes(searchText.toLowerCase()) ||
      (achievement.participants &&
        achievement.participants.some((p) =>
    ...
    if (dateRange && dateRange.length === 2) {
      ...
      if (achievement.date && achievement.date.isValid()) {
        matchesDate = achievement.date.isBetween(
          startDate,
          endDate,
          null,
          "[]"
    ...
    return (
      matchesSearch && matchesType && matchesLevel && matchesDate && matchesTab
    );
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (date) => (date ? date.format("YYYY-MM-DD") : "未知"),
    },
    {
      title: "状态",
      dataIndex: "status",
  ...
  return (
    <Layout>
      <Navbar />
      <Content
        style={{ padding: "24px", minHeight: "calc(100vh - 64px - 64px)" }}
      >
            ...
            >
              ...
              <Form.Item label="日期范围">
                <RangePicker
                  value={dateRange}
                  onChange={(dates) => {
                    // 处理日期格式问题：确保日期是有效的 moment 对象
                    if (dates && dates.length === 2) {
                      // 重新创建 moment 对象，确保格式正确
                      const formattedDates = [
                        moment(dates[0]?.format("YYYY-MM-DD")), // 重新格式化为标准日期
          ...
          >
            ...
            <TabPane
              tab={
                <Badge
                  count={
                    achievements.filter((a) => a.status === "rejected").length
                  }
                >
                  <span>未通过</span>
                </Badge>
            ...
            <div>
              <div style={{ marginBottom: 16 }}>
                ...
                >
                  ...
                  <Tag color="cyan">
                    {levels.find((l) => l.value === selectedAchievement.level)
                      ?.label || selectedAchievement.level}
                  </Tag>
                  <Tag
                    color={
                      statusMap[selectedAchievement.status]?.color || "gray"
                    }
                  >
                    {statusMap[selectedAchievement.status]?.text ||
                ...
                {selectedAchievement.recommendLevel !== undefined &&
                  selectedAchievement.recommendLevel !== null && (
                    <div
                      style={{
                        display: "flex",
                        alignItems: "center",
                    ...
                    >
                      ...
                      <Rate
                        value={Number(selectedAchievement.recommendLevel)}
                        disabled
                      />
                    </div>
                  )}
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <p style={{ whiteSpace: "pre-wrap" }}>
                  {selectedAchievement.description || "无描述信息"}
                </p>
              </div>
              ...
              {selectedAchievement.participants && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {selectedAchievement.participants.map(
                      (participant, index) => (
